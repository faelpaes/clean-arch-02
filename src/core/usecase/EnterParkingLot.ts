...
export default class EnterParkingLot {
    ...
    async execute(code: string, plate: string, date: Date) {
        ...
        if (!parkingLot.isOpen(parkedCar.arrival)) throw new Error("The parking lot is closed");
        if (parkingLot.isFull()) throw new Error("The parking lot is full");
        await this.parkingLotRepository.enterParkingLot(parkedCar.code, parkedCar.plate, parkedCar.arrival);
